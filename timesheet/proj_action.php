...
ChromePhp::log('project action page ', $action);
if ($action == "add" || $action == "edit") {

	
	$assigned = isset($_REQUEST["assigned"]) ? $_REQUEST['assigned']: array();

	ChromePhp::log('project action page ', $assigned);
...
/*	$title = addslashes($title);
	...
	ChromePhp::log('project action edit ', $assigned, $proj_id);
	if (isset($assigned)) {
		dbQuery("DELETE FROM $ASSIGNMENTS_TABLE WHERE proj_id=$proj_id");
		ChromePhp::log('project action edit before insert');
		/*while (list(,$username) = each($assigned)) {
			ChromePhp::log('inserting for');
			dbQuery("INSERT INTO $ASSIGNMENTS_TABLE VALUES ($proj_id, '$username', 1)");
		}*/
